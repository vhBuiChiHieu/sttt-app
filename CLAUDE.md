...
| `src/shared/ipc.ts` | IPC contract — 12 `CHANNELS` + payloads + `IpcApi`. Edit here first. |
...
`overlay:clickthrough-state` (main→control echo) · `overlay:appearance` ·
`settings:get` `settings:set` · `app:quit`. Event subscriptions return an `Unsubscribe`.
...
- **Overlay window spans the full work area** (`windows.ts createOverlay`), transparent +
  click-through by default. Because the surface is full-screen, the overlay body MUST stay
  transparent: `styles/index.css` paints `body { background: var(--bg) }` (opaque, shared with the
  control window), so `overlay.css` overrides `html,body,#root { background: transparent }` — else the
  whole screen tints dark. While *unlocked* (Ctrl+Alt+L) the full screen is a click dead-zone until re-locked.
- **`settingsStore` mixes data + action methods** (`set`/`hydrate`/`reset`) in one flat object, so a
  no-selector `useSettingsStore()` snapshot carries functions. Strip to a plain `Settings` (pick
  `DEFAULT_SETTINGS` keys) before the `settings:set` IPC — functions can't be structured-cloned
  ("An object could not be cloned"). See `control/App.tsx` `toPlainSettings`.
- **Click-through lock is centralized in `main/ipc.ts`** (`setClickThroughLocked` + module var); the
  Ctrl+Alt+L hotkey, tray, and the `overlay:set-clickthrough` IPC handler all route through it, and it
  echoes `overlay:clickthrough-state` so the control toggle never drifts.
...
This project is indexed by GitNexus as **sttt-app** (970 symbols, 2113 relationships, 53 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
